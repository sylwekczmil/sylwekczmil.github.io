...
export function checkerboard(n: number, noise: number, rnd: () => number): Sample[] {
  // 3×3 alternating tiles: multimodal, curved-free, a classic prototype-method
  // playground where single linear or axis-poor learners struggle
  const out: Sample[] = []
  for (let i = 0; i < n; i++) {
    const x = 0.03 + rnd() * 0.94
    const y = 0.03 + rnd() * 0.94
    const label = (Math.floor(x * 3) + Math.floor(y * 3)) % 2
    out.push({ x: x + gauss(rnd) * noise * 0.5, y: y + gauss(rnd) * noise * 0.5, label })
  }
  return out
}
...
export type DatasetKey = 'moons' | 'circles' | 'spiral' | 'xor' | 'checkerboard' | 'blobs'
...
> = {
  ...
  checkerboard: { name: 'checkerboard', gen: checkerboard },
